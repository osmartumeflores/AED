...
public class BTree {
    ...
    /**
     * Elimina una clave del árbol. Implementación parcial: solo en hojas.
     */
    public void eliminar(String clave) {
        eliminarEnHoja(raiz, clave);
    }

    private void eliminarEnHoja(BTreeNode nodo, String clave) {
        if (!nodo.hoja) {
            // Buscar en el hijo correspondiente
            int i = 0;
            while (i < nodo.claves.size() && clave.compareTo(nodo.claves.get(i)) > 0) i++;
            eliminarEnHoja(nodo.hijos.get(i), clave);
        } else {
            int index = nodo.claves.indexOf(clave);
            if (index != -1) {
                nodo.claves.remove(index);
                nodo.valores.remove(index);
                System.out.println("Producto con clave '" + clave + "' eliminado.");
            } else {
                System.out.println("La clave '" + clave + "' no se encuentra en la hoja.");
            }
        }
    }
